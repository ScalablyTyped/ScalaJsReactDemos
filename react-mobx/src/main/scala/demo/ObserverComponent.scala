...
import japgolly.scalajs.react.{facade, Children, CtorType, PropsChildren}
...
object ObserverComponent:
  private def create[P, C <: Children, CT[-p, +u] <: CtorType[p, u]](
    ...
    val jsRender =
      render.andThen(_.rawElement): js.Function1[Box[P] with facade.PropsWithChildren, facade.React.Element]
  ...
  def withChildren[P](
      render:   (P, PropsChildren) => VdomElement
  )(implicit s: CtorType.Summoner[Box[P], Children.Varargs]): Component[P, s.CT] =
