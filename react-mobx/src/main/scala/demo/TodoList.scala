...
case class TodoListProps(store: TodoStore, peopleStore: PeopleStore)

val TodoList = ObserverComponent { case TodoListProps(store, peopleStore) =>
  useEffect(
    () =>
